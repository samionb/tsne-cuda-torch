...
def joint_probabilities_from_squared_distances(
    ...
    return torch.clamp(p, min=MACHINE_EPSILON)
...
def build_sparse_affinity_from_knn(
    ...
    *,
    ...
    probs = binary_search_perplexity_torch(
        torch.as_tensor(distances_data, dtype=torch.float32, device=device),
        perplexity,
        verbose=verbose,
    )

    p = csr_array(
        (probs.cpu().numpy().ravel(), distances_nn.indices, distances_nn.indptr),
...
def build_sparse_affinity_from_precomputed(
    ...
    *,
    ...
    probs = binary_search_perplexity_torch(
        torch.as_tensor(sorted_distances, dtype=torch.float32, device=device),
        perplexity,
        verbose=verbose,
    ).cpu().numpy()

    indptr = np.arange(0, n_samples * n_neighbors + 1, n_neighbors, dtype=np.int64)
    p = csr_array((probs.ravel(), order.ravel(), indptr), shape=(n_samples, n_samples))
    p = p + p.T
    p /= max(p.sum(), MACHINE_EPSILON)
    rows = np.repeat(np.arange(n_samples, dtype=np.int64), np.diff(p.indptr))
    return SparseAffinity(matrix=p, rows=rows)
...
def build_sparse_affinity_from_sparse_precomputed(
    ...
    *,
    ...
    p = csr_array((probs.ravel(), distances.indices, distances.indptr), shape=distances.shape)
    p = p + p.T
    p /= max(p.sum(), MACHINE_EPSILON)
    rows = np.repeat(np.arange(n_samples, dtype=np.int64), np.diff(p.indptr))
    return SparseAffinity(matrix=p, rows=rows)
